...
DJ=DJI[,6] # <==Column 6 of the object "AAPL" in R
ret_dj=diff(log(DJ))
plot(DJ, col="blue")
plot(ret_dj, col="red")
...
#ret=removeNA(ret)
ret=na.omit(ret)
length(ret) #<===now we have 3415 observations for returns after dropping NA obs
...
mean(ret0) #mean
var(ret0) #variance
stdev(ret0) # standard deviation
sd(ret0) #standard deviation
skewness(ret0) #skewness
kurtosis(ret0)  #<==(Kurtosis-3)
...
legend("right", inset=0.03, legend=c("annual","monthly","vol_s94","vol_s99"),pch=1,  col=c(1,2,3,4), horiz=F)

#################################
#GARCH Estimates of Volatility
#################################

library(fGarch)
m2=garchFit(~garch(1,1),data=ret,trace=F) # Fit an GARCH(1,1) model
summary(m2)
plot(m2)
1
2
...
0
#str(m2)
?garchFit # check out help for description of output

## Annualized Volatility using daily standard deviation from GARCH:
v1=m2@sigma.t*sqrt(252)*100 # annualized volatility in %

#Plot GARCH volatility
plot(v1, type='l') #<--simple plot with no time series

#Plot volatility using xts time series object
vol_garch=as.xts(v1, dates)
plot(vol_garch,xlab='year',ylab='volatility',type='l')

## Standardized residuals
resi=residuals(m2,standardize=T) # Standardized residuals

library(fBasics)   
basicStats(ret)  #<== Compute descriptive statistics of returns
basicStats(resi)  #<== Compute descriptive statistics of Standardized residuals
#kurtosis(resi) #<== excess kurtosis




#m2=garchFit(~garch(1,1),data=ret)  # Fit GARCH(1,1) lots of output
#summary(m2)

m3= garchFit(~ garch(1,0), data = ret, trace=F) # Fit an ARCH(1) model
summary(m3)
m5= garchFit(~ garch(4,1), data = ret,trace=F) ## Fit GARCH(4,1)
summary(m5)
m6=garchFit(~ garch(1,1), data = ret,trace=F)  # Fit GARCH(1,1)
summary(m6)
m7=garchFit(~garch(1,1), data = ret,trace=F, cond.dist="std") #<--Fit GARCH(1,1) with t-dsitribution
summary(m7)

##long run variance
#sig2 = m7@fit$par[2]/(1-m7@fit$par[3]-m7@fit$par[4])
#vol_u=100*sqrt(252*sig2)
#vol_u

## Forecast out-of-sample for volatility for k days ahead
k=252 # 1 year forecast
sigf=predict(m7,k)
head(sigf,10)
volf=sqrt(252)*sigf[,3]*100

#Plot volatility forecast 
plot(volf,ylab='volatility forecast',type='l')

#predict(m7, n.ahead=k,plot=TRUE)
