...
#setwd("~/Documents/Teaching/FIN325_current/module1")
...
# import JPM.xls data from a file
library(readxl)
JPM=read_excel("JPM.xls")
dim(JPM)
price=JPM$JPM_adjclose
date=as.Date(JPM$Date)
price_JPM=xts(price,date)

plot(price_JPM, type='l', col='blue')
ret_JPM=diff(log(price_JPM))
plot(ret_JPM)

#IBM Data
library(readxl)
IBM <- read_excel("IBM.xls")
View(IBM)

price=IBM$IBM
dates=IBM$Date
#Convert to time  Series using either xts or zoo library
library(xts)
dates=as.Date(dates)
price_ibm=xts(price,dates) # the first argument is a vector or matrix of time series, the second argument is vector of dates
plot(price_ibm, col='blue')
ret_ibm=diff(log(price_ibm))
plot(ret_ibm)
